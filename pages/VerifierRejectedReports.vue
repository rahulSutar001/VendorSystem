...
<template>
  <div class="">
    ...
    <div class=" sm:hidden grow ml-7 mt-8 text-center bg-white pb-1 ">
            <button class="border-b-2 cursor-default font-semibold text-md lg:text-lg md:text-base  ">Rejected Reports</button>
        </div>

        <div class="flex flex-wrap flex-row  w-full py-2 sm:mt-8 mt-0">

            <div class=" sm:inline-flex hidden grow ml-5 mt-1">
                <button class="border-b-2 cursor-default font-semibold text-sm lg:text-lg md:text-base  ">Rejected Reports</button>
            </div>
            ...
            <tbody>
              ...
              <tr v-for="( data, index) in paginatedData" :key="index">
                ...
                <td>
                  <div class="">
                    <div><button
                        class=" flex w-20 cursor-default justify-center bg-red-600  text-gray-100 px-1 py-1 mx-1   rounded-sm tracking-wide font-sm  shadow-sm  ">{{
                          data.Verifier }}</button></div>
                  </div>
                </td>
                <td>
                  <div class="icon">
        ...
        <div class="text-center mt-5">
          ...
          <div class="">
            <div class=" text-center">
              <div>
                <button
                  class=" flex justify-center bg-red-600  hover:bg-red-400 text-gray-100 px-1 py-1 mx-1   rounded-sm tracking-wide font-sm  shadow-sm cursor-pointer transition ease-in duration-200 ">{{
                    data.Verifier }}</button>
              </div>
            </div>
          </div>
...
//get method
// const { data: resDataSuccess } = await useFetch('api/fetchreport/fetchreportauthoriser_rejected', {
//   method: 'POST',

// })
// Data = resDataSuccess
// below code is to get data from datandtime format 
onMounted(() => {

  const resDataSuccess = $fetch("/api/fetchreport/fetchreportauthoriser_rejected")
    .then((res) => {
      Data.value = res
    })
