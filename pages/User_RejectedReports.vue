            ...
            <tbody>
              ...
              <tr v-for="( data, index) in paginatedData" :key="index">
                ...
                <td>
                  <div class="">
                    <div><button
                        class=" flex justify-center bg-red-600  text-gray-100 px-1 py-1 mx-1   rounded-sm tracking-wide font-sm  shadow-sm  ">{{
                          data.status }}</button></div>
                  </div>
                </td>
                <td>
                  <div class="icon">
                    <span><label for="my-modal-4" class=""><i class="fa fa-eye"
...
const update = async () => {
  ...
  await useFetch('/api/module1/UserAPI/Update', {
    method: 'PUT',
...
//get method
// const { data: resDataSuccess } = await useFetch('api/fetchreport/fetchreportsforuser', {
//   method: 'POST',
...
onMounted(async () => {
  ...
  const resDataSuccess = $fetch("/api/fetchreport/fetchreportRejected_User")
    .then((res) => {
      Data.value = res
