<template>
  <div class="">
    ...
    <div class="  ml-5 w-full rounded-2xl">
            ...
            <tbody>
              ...
              <tr v-for="( data, index) in paginatedData" :key="index">
                ...
                <td>
                  <div class="flex">
                    <div><button
                        class=" flex w-20 justify-center bg-green-500  hover:bg-green-400 text-white px-1 py-1 mx-1   rounded-sm tracking-wide font-sm  shadow-sm cursor-pointer transition ease-in duration-200 "
                        @click.prevent="approvereport(data.Id)">Approve</button></div>
                    <div><button
                        class=" flex w-20 justify-center bg-red-500 hover:bg-red-400 text-white px-1 py-1 mx-1 rounded-sm  tracking-wide font-sm  shadow-sm cursor-pointer transition ease-in duration-200 "
                        @click.prevent="rejectreport(data.Id)">Reject</button></div>
                  </div>
                </td>
                <td>
                  <div class="icon">
      ...
      <!-- mx-16 -->
      <div class="overflow-x-auto text-black bg-white border-2 my-5 px-5 py-3 sm:text-xs smallscreentable"
        ...
        <div class="text-center mt-5">
          ...
          <div class="flex flex-wrap justify-center">
            <div><button
                class=" flex justify-center bg-green-500  hover:bg-green-400 text-gray-100 px-1 py-1 mx-1   rounded-sm tracking-wide font-sm  shadow-sm cursor-pointer transition ease-in duration-200 "
                @click="approvereport(data.Id)">Approve</button></div>
            <div><button
                class=" flex justify-center bg-red-500  hover:bg-red-400 text-gray-100 px-1 py-1 mx-1 rounded-sm tracking-wide font-sm  shadow-sm cursor-pointer transition ease-in duration-200 "
...
async function approvereport(value: any) {
  return await $fetch('/api/Decision_Api/decision_admin_approved_rejected', {
    method: 'POST',
    body: {
      "ReportId": value,
      "Approve": "True",
...
//get method
onMounted(async () => {
  const resDataSuccess = await $fetch('/api/Verifier_Api/Getreport_to_admin', {
    method: 'GET',

  })
  Data.value = resDataSuccess
