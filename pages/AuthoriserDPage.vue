<template>
    <div class="">
        ...
        <div class=" ml-5 w-full rounded-2xl ">
                            ...
                            <tr v-for="( data, index) in paginatedData" :key="index">
                                ...
                                <td>
                                    <div class="flex">
                                        ...
                                        <div><button
                                                class=" flex w-20 justify-center bg-red-500  hover:bg-red-400 text-gray-100 px-1 py-1 mx-1 rounded-sm tracking-wide font-sm  shadow-sm cursor-pointer transition ease-in duration-200 "
            ...
            <!-- model starts here  -->
            <div>
                <input type="checkbox" id="my-modal-4" class="modal-toggle rounded-none" />
                <label for="my-modal-4" class="modal cursor-pointer rounded-none">
...
const Secondsearchbar = () => {
    secondsearchbar.value = !secondsearchbar.value
}

// Report 3 Approve or rejected By Authoriser
async function approvereport(value: any) {
   
    await $fetch('/api/Decision_Api/decisionauthoriserapproved', {
        method: 'POST',
        body: {
            "ReportId": value,
            "Approve": "True",
...
onMounted(() => {
    ...
    const resDataSuccess = $fetch("/api/Authoriser_APi/Getreport_to_Authoriser")
        .then((res) => {
            Data.value = res
